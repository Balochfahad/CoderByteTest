...
let val = json.data;
// write code here
console.log((val.match(/age=32/g) || []).length);

let d = val.split(',');

let arr = []
d.map(item => {
    let innerVal = item.split('=');
    let obj = {};
    obj[innerVal[0]] = innerVal[1];
    arr.push(obj);
})

fs.writeFileSync('output.txt', d.join(',') , 'utf-8');
fs.readFileSync('./output.txt').toString().split(',').forEach(function (line) { 
